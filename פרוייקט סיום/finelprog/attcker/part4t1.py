...
def predict_from_pcap(pcap_file):
    ...
    if new_record is None:
        return "Unknown"  # If the pcap file couldn't be processed, return "Unknown"
...
def predict_on_random_samples(directory_path, num_iterations=100):
    ...
    print(f"Correct predictions: {count_t} out of {num_iterations} ({(count_t / num_iterations) * 100:.2f}%)")
...
try:
    ...
    print(f"Error: Unable to read the training CSV file: {e}")
    exit(1)  # Exit the program if reading the CSV file fails
...
# for 1 time predict
predicted_label = predict_from_pcap("chroo1.pcapng")
print(f"the label predict: {predicted_label}")
