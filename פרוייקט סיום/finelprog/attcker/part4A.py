...
def process_pcapng(file_path):
    try:
        packets = rdpcap(file_path)  # Reading the pcapng file
    ...
    # calculating the mean of all the packets in the record:
...
def process_directory(directory_path):
    ...
    for file_name in os.listdir(directory_path):
        if file_name.endswith(".pcapng"):  # if the file is from type pcapng
            file_path = os.path.join(directory_path, file_name)

            # choose the label of the record
